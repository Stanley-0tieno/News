...
export default function DashboardPage() {
    const topStory = FEATURED_ARTICLE;
                ...
                <div className="container mx-auto px-6 lg:px-12 relative z-10 flex flex-col lg:flex-row gap-12 lg:gap-8 items-center lg:items-end flex-grow pb-12">
                    <div className="lg:w-7/12 pt-8 pb-8 lg:pb-16 flex flex-col justify-center h-full">
                        <div className="flex gap-3 mb-8">
                            <span className="bg-[#3B82F6] text-white text-[11px] font-bold px-3 py-1 uppercase tracking-widest rounded shadow-sm">Markets & Industries</span>
                            <span className="bg-[#3B82F6] text-white text-[11px] font-bold px-3 py-1 uppercase tracking-widest rounded shadow-sm">Tech</span>
                        </div>
                ...
                <div className="container mx-auto px-6 lg:px-12 pb-12 relative z-20">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mt-4 pt-8 border-t border-gray-800/80">
                        {topArticles.map(article => (
                            <Link href={`/articles/${article.slug}`} key={article.id} className="flex gap-4 items-center group cursor-pointer">
                                <img src={article.imageUrl} alt={article.title} className="w-20 h-20 object-cover rounded shadow-md group-hover:shadow-lg transition-all" />
                                <div className="flex-1">
                                    ...
                                    <div className="flex flex-wrap gap-2 items-center text-[11px] font-bold">
                                        <span className="bg-[#3B82F6] text-white px-2 py-0.5 rounded-sm uppercase tracking-wider">{article.category.name}</span>
                                        <span className="text-gray-400 font-medium tracking-wide">• 7 hours ago</span>
                                    </div>
                                </div>
                            </Link>
                        ))}
