...
export default async function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans w-full">
      ...
      <section className="bg-[var(--brand-bg)] text-[var(--brand-text)] pt-16 pb-0 relative overflow-hidden flex flex-col justify-between min-h-[500px] lg:min-h-[700px]">
        {topStory && (
          <div className="container mx-auto px-6 lg:px-12 relative z-10 flex flex-col lg:flex-row gap-12 lg:gap-8 items-center lg:items-end flex-grow pb-12">
            <div className="lg:w-7/12 pt-8 pb-8 lg:pb-16 flex flex-col justify-center h-full">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-7xl font-bold leading-[1.1] mb-8 lg:mb-10 text-[var(--brand-text)]">
                {topStory.title}
              </h1>
              <p className="text-lg md:text-xl text-[var(--brand-muted)] mb-10 max-w-2xl leading-relaxed">
                {topStory.summary}
              </p>
              <Link href={`/articles/${topStory.slug}`} className="inline-flex items-center text-[var(--brand-text)] font-bold hover:text-[var(--brand-primary)] transition-colors group text-lg tracking-wide">
            ...
            <div className="lg:w-5/12 flex justify-center lg:justify-end items-end relative w-full lg:absolute lg:right-0 lg:bottom-0 h-full">
              ...
              <div className="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-[var(--brand-bg)] to-transparent lg:hidden"></div>
            </div>
          </div>
        )}
        ...
        <div className="container mx-auto px-6 lg:px-12 pb-12 relative z-20">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mt-4 pt-8 border-t border-[var(--brand-border)]">
            {topArticles.map((article: Article) => (
              <Link href={`/articles/${article.slug}`} key={article.id || article.slug} className="flex gap-4 items-center group cursor-pointer">
                ...
                <div className="flex-1">
                  <h4 className="text-[15px] font-bold text-[var(--brand-text)] leading-tight line-clamp-2 group-hover:text-[var(--brand-primary)] transition-colors duration-200 mb-2">
                    {article.title}
                  </h4>
                  <div className="flex flex-wrap gap-2 items-center text-[11px] font-bold">
                    ...
                    <span className="text-[var(--brand-muted)] font-medium tracking-wide">•
      ...
      <section className="py-24 px-6 lg:px-12 bg-[var(--brand-bg)]">
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="flex justify-center border-t border-[var(--brand-border)] pt-16">
            <button className="border-2 border-[var(--brand-border)] bg-[var(--brand-card)] hover:bg-[var(--brand-bg)] text-[var(--brand-text)] py-4 px-16 rounded-xl font-bold text-[13px] uppercase tracking-widest transition-colors shadow-sm w-full max-w-3xl">
              View More
            </button>
          </div>
        </div>
      ...
      <section className="py-24 px-6 border-y border-[var(--brand-border)] bg-[var(--brand-card)]">
        <div className="container mx-auto max-w-6xl">
          <div className="flex flex-col items-center justify-center mb-16 text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-[var(--brand-text)] mb-6">Our Impact in Numbers</h2>
            <p className="text-[var(--brand-muted)] text-lg max-w-2xl leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-center divide-y md:divide-y-0 md:divide-x divide-[var(--brand-border)]">
            <div className="pt-8 md:pt-0">
              <p className="text-5xl lg:text-7xl font-bold text-[#3B82F6] mb-4">
                <AnimatedCounter value={2} suffix=".4M" duration={1500} />
              </p>
              <p className="text-[var(--brand-muted)] font-bold uppercase tracking-widest text-sm">Total Views</p>
            </div>
            <div className="pt-8 md:pt-0">
              ...
              <p className="text-[var(--brand-muted)] font-bold uppercase tracking-widest text-sm">Active Subscribers</p>
            </div>
            <div className="pt-8 md:pt-0">
              <p className="text-5xl lg:text-7xl font-bold text-[#3B82F6] mb-4">
                <AnimatedCounter value={142} duration={2500} />
              </p>
              <p className="text-[var(--brand-muted)] font-bold uppercase tracking-widest text-sm">Published Articles</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-28 px-6 bg-[var(--brand-bg)]">
        <div className="container mx-auto text-center max-w-4xl">
          <h2 className="text-4xl md:text-5xl font-bold text-[var(--brand-text)] mb-8 tracking-tight">
            Publication & Newsletter Template for Figma
          </h2>
          <p className="text-[var(--brand-muted)] text-lg md:text-xl mb-12 max-w-2xl mx-auto leading-relaxed">
          ...
          <div className="bg-[var(--brand-card)] p-8 rounded-3xl shadow-sm border border-[var(--brand-border)] max-w-2xl mx-auto mb-16">
            <h3 className="text-lg font-bold text-[var(--brand-text)] mb-4">Get the latest news directly in your inbox</h3>
            <form className="flex flex-col sm:flex-row gap-3">
              <input
                type="email"
                placeholder="Email Address"
                className="px-6 py-4 rounded-xl border border-[var(--brand-border)] bg-[var(--brand-bg)] text-[var(--brand-text)] w-full focus:outline-none focus:ring-2 focus:ring-[#f97316]/50 transition-shadow text-[15px]"
                required
              />
              <button
                type="button"
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            ...
            <button className="bg-[var(--brand-card)] text-[var(--brand-text)] border-2 border-[var(--brand-border)] px-10 py-4 rounded-xl font-bold hover:bg-[var(--brand-bg)] transition-colors shadow-sm w-full sm:w-auto text-[15px] tracking-wide">
              Contact Us
            </button>
          </div>

          <div className="mt-20 flex items-center justify-center opacity-50">
            <div className="h-px bg-[var(--brand-border)] w-16"></div>
            <div className="mx-4 flex gap-1 text-[var(--brand-muted)]">
